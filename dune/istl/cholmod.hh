...
public:
   ...
   * In the case of a given ignore field a mapping from the flat ordered indices was computed and stored
   * in inverseSubIndices.
   * Ignored entries will stay untouched both in x and b in this method.
  */
  void apply(X& x, B& b, InverseOperatorResult& res)
  {
    ...
    // copy to cholmod
    auto bp = b2.get();
    if (inverseSubIndices_.size() == 0) // no ignore field given
    {
   ...
   * like the BlockVector template of the class. It will ignore both rows and cols of the matrix.
   *
