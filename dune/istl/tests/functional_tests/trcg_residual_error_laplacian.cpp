...
namespace
{
  ...
  using Solver = Dune::TCGSolver<BVector,BVector,Dune::KrylovTerminationCriterion::ResidualBased>;


  struct TestTRCG_ResidualBasedTermination : ::testing::Test
  {
    TestTRCG_ResidualBasedTermination()
      : mat(),
        A( mat )
    {}
...
TEST_F(TestTRCG_ResidualBasedTermination, Laplacian)
{
  BVector b(N*N), x(N*N);
  ...
  TestSetup::compareWithStoredSolution(x, TestSetup::tol*TestSetup::condition);

  ASSERT_TRUE( res.converged );
  ASSERT_EQ( res.iterations, TestSetup::expectedIterations_ResidualBasedError );
}
