...
namespace Dune {
  ...
  std::shared_ptr<ScalarProduct<X>> createScalarProduct(std::shared_ptr<const Comm> comm, SolverCategory::Category category)
  {
    switch(category)
    {
  ...
  { return createScalarProduct(stackobject_to_shared_ptr(comm), category); }
