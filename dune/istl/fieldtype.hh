...
struct FieldType<MultiTypeBlockVector<Args...>, false>
{
  ...
  using Block0 = typename std::decay_t<decltype(std::get<0>(std::declval<V>()))>;
  ...
  static constexpr bool hasUniqueFieldType = []{
    ...
    forEach(integralRange(index_constant<sizeof...(Args)>()), [&](auto&& i) {
      using FT = typename std::decay_t<decltype(std::get<i>(std::declval<V>()))>;
...
struct FieldType<MultiTypeBlockMatrix<FirstRow, Args...>, false>
{
  ...
  using Block00 = typename std::decay_t<decltype(std::get<0>(std::get<0>(std::declval<M>())))>;
  using FieldType00 = typename FieldType<Block00>::type;
  static constexpr bool hasUniqueFieldType = []{
    bool isSameType = true;
    using namespace Dune::Hybrid;
    forEach(integralRange(index_constant<1+sizeof...(Args)>()), [&](auto&& i) {
      using namespace Dune::Hybrid; // needed for icc, see issue #31
      forEach(integralRange(index_constant<FirstRow::size()>()), [&](auto&& j) {
        using FT = typename std::decay_t<decltype(std::get<j>(std::get<i>(std::declval<M>())))>;
