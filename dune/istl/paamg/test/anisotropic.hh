...
void fillValues(int N, M& mat, int overlapStart, int overlapEnd, int start, int end, T eps)
{
  ...
  Dune::Hybrid::ifElse(Dune::IsNumber<Block>(),
    ...
    [&](auto id) {
      std::fill(id(dval).begin(), id(dval).end(), 2.0+2.0*eps);
      std::fill(id(bone).begin(), id(bone).end(), 1.0);
      std::fill(id(bmone).begin(), id(bmone).end(), -1.0);
      std::fill(id(beps).begin(), id(beps).end(), -eps);
    });
