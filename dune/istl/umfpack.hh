...
namespace Dune {
  ...
  template<typename T, typename A, int n, int m>
  class UMFPack<BCRSMatrix<FieldMatrix<oc::OpCounter<T>,n,m>,A > >
      : public InverseOperator<
          BlockVector<FieldVector<oc::OpCounter<T>,m>,
                      typename A::template rebind<FieldVector<oc::OpCounter<T>,m> >::other>,
    BlockVector<FieldVector<oc::OpCounter<T>,n>,
                            typename A::template rebind<FieldVector<oc::OpCounter<T>,n> >::other> >
  {
    public:
    /** @brief The matrix type. */
    typedef Dune::BCRSMatrix<FieldMatrix<oc::OpCounter<T>,n,m>,A> Matrix;
    typedef Dune::BCRSMatrix<FieldMatrix<oc::OpCounter<T>,n,m>,A> matrix_type;

    using BaseMatrix = BCRSMatrix<FieldMatrix<T,n,m>,A>;

    /** @brief The corresponding SuperLU Matrix type.*/
    typedef Dune::ColCompMatrix<BaseMatrix> UMFPackMatrix;
    /** @brief Type of an associated initializer class. */
    typedef ColCompMatrixInitializer<BaseMatrix> MatrixInitializer;
    /** @brief The type of the domain of the solver. */
    typedef Dune::BlockVector<
      FieldVector<oc::OpCounter<T>,m>,
      typename A::template rebind<FieldVector<oc::OpCounter<T>,m> >::other> domain_type;
    /** @brief The type of the range of the solver. */
    typedef Dune::BlockVector<
      FieldVector<oc::OpCounter<T>,n>,
      typename A::template rebind<FieldVector<oc::OpCounter<T>,n> >::other> range_type;
    ...
    void setMatrix(const Matrix& matrix)
    {
      if ((umfpackMatrix_.N() + umfpackMatrix_.M() > 0) || matrixIsLoaded_)
        free();
      umfpackMatrix_ = reinterpret_cast<const BaseMatrix&>(matrix);
      decompose();
    }
